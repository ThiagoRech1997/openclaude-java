...
- **tools** — `Tool` interface (`name()`, `description()`, `inputSchema()`, `execute()`), `ToolRegistry`, and `ToolUseContext` (passed to every `execute()` — tracks per-session state such as files read, used by `FileWriteTool`). Built-in tools are split by domain under `dev.openclaude.tools.<domain>`: `bash/`, `fileread/`, `filewrite/`, `fileedit/`, `glob/`, `grep/`, `webfetch/`, `websearch/`, `agent/`, `monitor/`, `kill/`, `todo/` (session todo list), `notebook/` (Jupyter `.ipynb` editor), `plan/` (plan-mode exit flow), `toolsearch/` (deferred schema loading), `notify/` (PushNotification + providers), `worktree/` (`WorktreeSession` + Enter/ExitWorktree), `background/` (shared `BackgroundProcessManager`). See **Built-in Tools** below for per-tool contracts.
...
- **mcp** — MCP 2024-11-05 client. `McpClientManager` manages server connections (stdio subprocess transport, plus HTTP/SSE for remote servers via `HttpSseTransport` — a url-only config defaults to SSE), `McpToolBridge` wraps remote MCP tools as native `Tool` instances. Tool names are prefixed `mcp__<server>__<tool>`.
...
- **NotebookEditTool** — edits Jupyter notebooks (`.ipynb`) cell-by-cell. `edit_mode` ∈ `replace`/`insert`/`delete`/`move`; cells are addressed by 0-based `cell_index` (and `new_index` for `move`). Preserves notebook metadata and untouched cells; when replacing a `code` cell's source, clears that cell's `outputs` and `execution_count` to avoid stale results. Uses Jackson to parse/serialize; writes pretty-printed JSON back to disk.
...
- **ToolSearchTool** — deferred schema loading. When the catalog exceeds `OPENCLAUDE_TOOL_SEARCH_THRESHOLD` (default 25), MCP tools are registered deferred: excluded from the API tools array, listed as a name+summary index inside ToolSearch's own (dynamic) description. `ToolSearch(query)` (`select:Name,...` or keywords) activates matches in the `ToolRegistry`; the engine rebuilds the array every iteration, so they become callable on the next LLM call.
- **PushNotificationTool** — `PushNotification(title, body, level)` fans out to the providers configured in `settings.json#notifications` (`stdout` default, `libnotify`, generic `webhook`, `slack`); URL/header values support `${ENV_VAR}`.
- **EnterWorktreeTool / ExitWorktreeTool** — move the session into/out of a temporary git worktree at runtime. The mutable cwd lives in `WorkspaceState`, shared per engine via `ToolUseContext.workspace()` — `workingDirectory()` always reflects the current location. Exit keeps the worktree+branch when it has changes (unless `keep_changes: false`).
- **ExitPlanModeTool** — plan-mode exit flow. Only usable while `PermissionMode.PLAN` is active (in PLAN, `QueryEngine` also appends a dedicated planning system prompt each iteration). The model calls it with the finished `plan` (markdown); the REPL renders the plan and asks for approval — approving switches the mode back to DEFAULT, rejecting keeps PLAN. `isReadOnly() = true` so the plan-mode gate lets it run; without an interactive approval handler (print/serve) it refuses to exit plan mode.
